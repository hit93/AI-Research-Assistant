...
- **🧠 100% Textbook LangChain & LangGraph Multi-Agent Architecture**:
  - Declarative state machine via **LangGraph `StateGraph`** (`START ➔ planner ➔ retriever ➔ synthesizer ➔ verifier ➔ END`).
...
- **🔍 Automated Quality & Faithfulness Verification (LLM-as-Judge)**:
  ...
  - Powered by **`openai/gpt-oss-120b`** (120B MoE, 131K context, ~500 tok/s on Groq) — a dedicated, more capable judge model separately configurable from the synthesizer via `VERIFIER_MODEL` in `.env`.
  - Computes faithfulness score (0–100), citation accuracy, coverage, and clarity metrics.
...
```
Open your browser at **`http://localhost:8501`** (or the port shown in terminal). Features 4 dedicated tabs:
...
```
Outputs the complete query plan, retrieved sources count, structured synthesis report, and the automated verification audit with score and recommendations.
...
│   │   └── verifier.py      # LCEL chain: verifier_prompt | llm.with_structured_output(VerificationResult)
│   ├── graphs/              # Textbook LangGraph: Multi-agent StateGraph
│   │   ├── __init__.py
│   │   ├── state.py         # ResearchGraphState TypedDict schema (includes verification)
│   │   ├── nodes.py         # Graph nodes (plan_node, retrieve_node, synthesize_node, verify_node)
...
├── tests/
│   ├── test_graphs.py       # LangGraph state machine & node unit tests (7 tests)
│   ├── test_agents.py       # Facades, chains, verifier, schemas & tool tests (21 tests)
│   ├── test_config.py       # Settings & environment validation tests (3 tests)
│   └── test_tools.py        # Tool normalization & retrieval tests (5 tests)
...
| **Phase 3** | Reasoning & Agentic Pipeline | 🟢 Completed | LangChain LCEL, LangGraph `StateGraph` (`START ➔ planner ➔ retriever ➔ synthesizer ➔ verifier ➔ END`), 36 tests pass; LLM-as-judge self-critique |
